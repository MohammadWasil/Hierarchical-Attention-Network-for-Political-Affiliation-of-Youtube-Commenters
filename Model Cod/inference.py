...
def inference_data(dataloader_inference, model, device):
    ...
    with torch.no_grad():    
        for text, author in tqdm(dataloader_inference):
            text = text.to(device)
            ...
            if predicted_label == 0:
                authors_dictionary[author[0]] = "Liberal"
                left += 1
                left_comment += text.shape[1]
            elif predicted_label == 1:
                authors_dictionary[author[0]] = "Conservative"
                right += 1
                right_comment += text.shape[1]
            
    # returns the accuracy of the model
    return left, right, left_comment, right_comment, authors_dictionary